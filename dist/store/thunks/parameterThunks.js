...
import { selectApiInitialized } from '../slices/apiSlice'; // Import API selectors
...
import { setDefinedConditions, setProcessedConditions, setConditionalPartEffectsBatch, clearConditionalPartEffects } from '../slices/parametersSlice';
...
export const initializeConditionsAndParameters = createAsyncThunk('parameters/initializeConditions', async (rawConditions, { dispatch, getState, rejectWithValue }) => {
        ...
        if (partId !== null && parameterName) {
            if (rawCond.operator && rawCond.action && rawCond.action_value !== undefined) {
                processedConditions.push({
                    ...
                    operator: rawCond.operator,
                    valueToCompare: rawCond.value,
                    action: rawCond.action,
...
export const fetchParametersForPart = createAsyncThunk('parameters/fetchForPart', // Action type prefix
...
export const fetchParametersForReferencedParts = createAsyncThunk('parameters/fetchForReferencedParts', async (partIds, { getState, rejectWithValue }) => {
    const state = getState();
    const apiInitialized = selectApiInitialized(state);
    ...
    try {
        const results = {}; // UPDATED type for results
        ...
        settledIndividualPromises.forEach(result => {
            ...
            if (result.status === 'fulfilled') {
                const parameters = result.value; // value from .then
                if (parameters === null) {
                ...
                const errorMsg = (errorReason === null || errorReason === void 0 ? void 0 : errorReason.message) || String(errorReason);
...
export const updateParameterValue = createAsyncThunk('parameters/updateValue', async ({ partId, paramName, value }, { getState, rejectWithValue }) => {
    const state = getState();
    ...
    try {
        ...
        const parameterToUpdate = parametersForPart.find((p) => { var _a; return ((_a = p.template_detail) === null || _a === void 0 ? void 0 : _a.name) === paramName; });
...
export const evaluateAndApplyConditions = createAsyncThunk('parameters/evaluateAndApplyConditions', async (_, { dispatch, getState }) => {
    var _a, _b;
    ...
    const mergeEffect = (partId, effect) => {
        ...
        newEffects[partId] = Object.assign(Object.assign({}, newEffects[partId]), effect);
    };
    for (const condition of processedConditions) {
        const paramValue = (_b = (_a = parameterValues[condition.partId]) === null || _a === void 0 ? void 0 : _a[condition.parameterName]) === null || _b === void 0 ? void 0 : _b.data;
        ...
        switch (condition.operator) {
            case 'equals':
                conditionMet = String(paramValue) === String(condition.valueToCompare);
                break;
            case 'not_equals':
                conditionMet = String(paramValue) !== String(condition.valueToCompare);
                break;
            case 'contains':
                conditionMet = String(paramValue).includes(String(condition.valueToCompare));
                break;
            case 'exists':
                conditionMet = paramValue !== undefined && paramValue !== null && paramValue !== '';
                break;
            case 'is_empty':
                conditionMet = paramValue === undefined || paramValue === null || paramValue === '';
                break;
            case 'greater_than':
                conditionMet = parseFloat(paramValue) > parseFloat(condition.valueToCompare);
                break;
            case 'less_than':
                conditionMet = parseFloat(paramValue) < parseFloat(condition.valueToCompare);
                break;
        ...
        if (conditionMet) {
            ...
            if (targetPartPks.length > 0) {
                ...
                switch (condition.action) {
                    case 'filter':
                        effectToApply.isVisible = condition.actionValue === 'show';
                        break;
                    case 'highlight':
                        effectToApply.highlight = condition.actionValue;
                        break;
                    case 'text_color':
                        effectToApply.textColor = condition.actionValue;
                        break;
                    case 'border':
                        effectToApply.border = condition.actionValue;
                        break; // Assuming actionValue is a valid CSS border string
                    case 'icon':
                        effectToApply.icon = condition.actionValue;
                        break;
                    case 'badge':
                        effectToApply.badge = condition.actionValue;
                        break;
...
//# sourceMappingURL=parameterThunks.js.map