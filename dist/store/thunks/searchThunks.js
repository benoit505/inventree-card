...
export const performSearch = createAsyncThunk('search/perform', // Action type prefix
async (query, thunkAPI) => {
    ...
    try {
        ...
        const results = await inventreeApiService.getParts({ search: query });
        // Map full results to the simplified SearchResultItem shape
        const simplifiedResults = results.map(part => ({
            pk: part.pk,
            name: part.name,
...
export const searchInventree = createAsyncThunk('search/searchInventree', async (query, { getState, rejectWithValue }) => {
    ...
    try {
        ...
        const mockResults = query.toLowerCase().includes('resistor') ?
...
*/ 
//# sourceMappingURL=searchThunks.js.map