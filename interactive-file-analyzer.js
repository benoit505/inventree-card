...
function detectCircularDependencies() {
  ...
  // Build dependency graph from fileData
  fileData.forEach(file => {
    ...
    file.imports.forEach(imp => {
      ...
      const importingFile = path.dirname(path.join(rootDir, filePath));
      const resolvedPath = path.resolve(importingFile, importPath);
      
      // Find the matching file in our fileData
      const matchingFile = fileData.find(f => {
        const fullPath = path.join(rootDir, f.path);
        return fullPath === resolvedPath + '.ts' || 
               fullPath === resolvedPath + '.tsx' ||
               fullPath === resolvedPath + '/index.ts';
      });
      
      if (matchingFile) {
        graph[filePath].push(matchingFile.path);
      }
    });
  });
...
// Generate coupling matrix between components
function generateCouplingMatrix() {
  // Extract component names (use directory names as components)
  const components = [...new Set(fileData.map(file => {
    const parts = file.path.split('/');
    return parts.length > 0 ? parts[0] : 'root';
  }))];
  
  components.forEach(compA => {
    couplingMatrix[compA] = {};
    
    components.forEach(compB => {
      if (compA === compB) {
        couplingMatrix[compA][compB] = 0;
        return;
      }
      
      // Count calls between components
      const calls = functionCalls.filter(call => {
        const callerComponent = call.callerFile.split('/')[0];
        const calleeFunc = allFunctions.find(f => f.name === call.callee);
        const calleeComponent = calleeFunc ? calleeFunc.filePath.split('/')[0] : '';
        
        return callerComponent === compA && calleeComponent === compB;
      });
      
      couplingMatrix[compA][compB] = {
        callCount: calls.length,
        dependencyStrength: calls.length > 0 ? Math.round(Math.log10(calls.length) * 10) / 10 : 0
      };
    });
  });
}
...
function analyzeCodebase() {
  ...
  // Process function calls safely with objects instead of Sets
  const callMap = {};
  ...
  try {
    ...
    functionCalls.forEach(call => {
      ...
      const funcName = String(call.callee);
      if (!callMap[funcName]) {
        callMap[funcName] = { 
          count: 0,
          callers: {} // Use an object instead of a Set
        };
      }
      callMap[funcName].count++;
      
      // Store caller in an object where we don't need .add()
      if (call.caller) {
        callMap[funcName].callers[String(call.caller)] = true;
      }
    });
  } catch (error) {
    console.error("Error collecting function call data:", error);
  }
  
  // Prepare the bottleneck data for display
  const bottlenecks = [];
  try {
    // Convert to array for sorting
    Object.entries(callMap).forEach(([funcName, data]) => {
      bottlenecks.push({
        name: funcName,
        callCount: data.count,
        callerCount: Object.keys(data.callers).length,
        bottleneckScore: data.count * Object.keys(data.callers).length
      });
    });
    ...
    console.error("Error processing bottleneck data:", error);
  }
  ...
  bottlenecks.slice(0, 20).forEach(func => {
...
// Run initial system analyses after files are processed
console.log("Processing system-level data...");
...
function analyzeFile(fileIndex) {
  ...
  if (incomingCalls.length > 0) {
    ...
    Object.keys(targetGroups).forEach(target => {
      ...
      targetGroups[target].forEach(call => {
        report += `| \`${call.caller}\` | \`${call.callerFile}\` | ${call.line} |\n`;
      });
    });
  }
  
  // Write files
  const sanitizedFilename = filePath.replace(/\//g, '_').replace(/\\/g, '_').replace(/\./g, '_');
  fs.writeFileSync(`${outputDir}/${sanitizedFilename}_analysis.md`, report);
  
  console.log(`Analysis complete! Results saved to:`);
  console.log(`  - ${outputDir}/${sanitizedFilename}_analysis.md`);
